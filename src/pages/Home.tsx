...
import { MOODS, MESSAGE_LIMIT, parseSongUrl } from '../../shared/rules';
import { api } from '../api';
...
export default function Home({ data, refresh, onSelect, notify }: Props) {
  ...
  async function send(e: FormEvent) {
    ...
    try {
      parseSongUrl(url);
      if (!moods.length) throw new Error('무드 태그를 하나 이상 선택해 주세요.');
      setBusy(true);
      const payload = { url, title, moods, message, current };
        ...
        <section className="compose-section">
          ...
          {pending ? (
            ...
            <form onSubmit={send} noValidate>
              ...
              <div className="field-label">
                <label>
                  어떤 순간에 듣나요? <span className="required">*</span>
                </label>
                <span>{moods.length} / 2 선택</span>
              </div>
                ...
                {MOODS.map((m) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      else if (moods.length < 2) setMoods([...moods, m]);
                      else notify('무드는 최대 2개까지 선택할 수 있어요.');
                    }}
                  >
              ...
              {advanced && (
                <div className="current-field">
                  ...
                  />
                  <p>같은 코드와 무드를 선택한 사람끼리 만나요. 비워두면 모두의 바다로 보내요.</p>
                </div>
              )}
              {error && (
